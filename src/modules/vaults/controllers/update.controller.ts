...
import { UpdateVaultRepository } from '../repositories/update.repository'
import { UpdateVaultUseCase } from '../use-cases/update.use-case'

export const updateVaultController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const updateVaultRepository = new UpdateVaultRepository(controllerInput.dbConnection, { session })
    ...
    // 3.2 update
    const response = await UpdateVaultUseCase.handle(
      {
        auth: verifyTokenResponse as IAuth,
        _id: controllerInput.httpRequest['params'].id,
        data: controllerInput.httpRequest['body'],
      },
      { schemaValidation, updateVaultRepository, retrieveVaultRepository, createActivityRepository, uniqueValidation },
    )
