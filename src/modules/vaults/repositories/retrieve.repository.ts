...
export interface IRetrieveVaultOutput {
  _id: string
  code: string
  name: string
...
export interface IRetrieveVaultRepository {
  handle(_id: string): Promise<IRetrieveVaultOutput>
}

export class RetrieveVaultRepository implements IRetrieveVaultRepository {
  constructor(
    public database: IDatabase,
    public options?: Record<string, unknown>,
  ) {}

  async handle(_id: string): Promise<IRetrieveVaultOutput> {
    ...
    return {
      ...
      racks: response.data[0]['racks'] as { code: string; name: string }[],
