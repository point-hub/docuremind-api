...
import { RetrieveDocumentUseCase } from '../use-cases/retrieve.use-case'

export const retrieveDocumentController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    // 3.2 retrieve
    const response = await RetrieveDocumentUseCase.handle(
